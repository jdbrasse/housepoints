...
if uploaded_file is not None:
    try:
        ...
        # Ensure only embedded staff initials count/appear
        ...
        # Merge with permanent list so everyone shows (0 if no points)
        ...
        cat_freq_conduct = (
            ...
            .rename(columns={"Points": "Frequency"}).sort_values("Frequency", ascending=False)
        ) if not conduct_df.empty else pd.DataFrame(columns=["Category","Frequency"])
        ...
        if lb_type == "House Points" and not house_df.empty:
            ...
            top15_hp = studs.sort_values("House Points", descending=True if hasattr(pd.Series, 'descending') else False).sort_values("House Points", ascending=False).head(15)
            st.dataframe(top15_hp[["Pupil Name", "Form", "House", "House Points"]], use_container_width=True)

            # Per House (Top 10) — with coloured headers and coloured dot in title
            ...
            # Per Form (Top 10) — choose header colour by the modal house in that form
